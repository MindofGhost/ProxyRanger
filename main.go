...
func findWorkingProxy(domain string) (string, bool) {
    ...
    ch, loaded := getOrCreateChannel(func() string { if chNeeded { return mainDom } else { return "" } }())
    if chNeeded && !loaded {
    ...
    if chNeeded {log.Printf("No working subdomain proxy for %s, waiting for mainDom check...", domain)}
